...
protected:
	...
	//debug printing (coordinates)
	char mouseStr[30];			// buffer to hold mouse coordinate data
	char mapStr[30];			// buffer to map coordinate data
	//how much the map is offset from the screen (should we create a Vector2 struct?)
	double offsetX;
	double offsetY;

	// Unsigned because they should -never- be negative since they range from 0 - mapSize.
	unsigned int focusedTileX;
	unsigned int focusedTileY;
